...
function answerFromTranscript(question, transcript) {
    const normalizedQuestion = question.toLowerCase();
    const lines = transcript.split("\n").filter((line) => line.trim());
    const questionWords = normalizedQuestion
        .split(/\s+/)
        .filter((word) => word.length > 3);
    const relevantLines = lines.filter((line) => questionWords.some((word) => line.toLowerCase().includes(word)));
...
function generateSummary(transcript) {
    ...
    const summaryLines = scoredLines
        ...
        .slice(0, 3)
        .map((item) => item.line);
    return (summaryLines.join(". ") || "Unable to generate summary from the transcript.");
}
function extractKeypoints(transcript) {
    ...
    const keypoints = lines
        ...
        const words = line.toLowerCase().split(/\s+/);
        const score = words.reduce((sum, word) => sum + (wordFreq.get(word) || 0), 0);
        const hasKeyPhrase = keyPhrases.some((phrase) => line.toLowerCase().startsWith(phrase));
        ...
        tools: [
            {
                ...
                description: "Fetch the transcript of a YouTube video given its URL. The transcript is cached for subsequent operations.",
                inputSchema: zodToJsonSchema(FetchTranscriptArgsSchema),
            },
            {
                name: "answer_question",
                description: "Answer a question based on the transcript of a YouTube video. Requires the video URL and the question.",
                inputSchema: zodToJsonSchema(AnswerQuestionArgsSchema),
            },
            {
                name: "generate_summary",
                description: "Generate a concise summary of a YouTube video's transcript based on its URL.",
                inputSchema: zodToJsonSchema(GenerateSummaryArgsSchema),
            },
            {
                name: "extract_keypoints",
                description: "Extract key points from a YouTube video's transcript based on its URL.",
                inputSchema: zodToJsonSchema(ExtractKeypointsArgsSchema),
            },
            {
                name: "chat_with_video",
                description: "Engage in a conversational chat about a YouTube video, maintaining context across messages.",
                inputSchema: zodToJsonSchema(ChatWithVideoArgsSchema),
            },
            {
                name: "create_video_notes",
                description: "Create a text file with comprehensive notes for a YouTube video, including summary, key points, and transcript excerpts. " +
                    "The file is saved in the 'notes' subdirectory with a generated or custom title.",
                inputSchema: zodToJsonSchema(CreateVideoNotesArgsSchema),
            },
            {
                name: "find_in_video",
                description: "Find where a specific phrase, sentence, or topic is discussed in a YouTube video's transcript. Returns timestamps and matching lines.",
                inputSchema: zodToJsonSchema(FindInVideoArgsSchema),
            },
            {
                name: "list_youtube_functionalities",
                description: "List all available YouTube-related functionalities provided by this agent, including tool names and descriptions.",
                inputSchema: zodToJsonSchema(ListYouTubeFunctionalitiesArgsSchema),
            },
        ],
    };
});
    ...
    try {
        ...
        switch (name) {
            ...
            case "chat_with_video": {
                ...
                const contextText = context.length > 1
                    ? `\n\nPrevious context:\n${context
                        .slice(0, -1)
                        .map((c) => `Q: ${c.question}\nA: ${c.answer}`)
                        .join("\n")}`
